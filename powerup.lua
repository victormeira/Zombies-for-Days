function powerupspawn(dt)
 ...
 if Shoe.numero == spawnnumero then
    Shoe.nomapa = 1
  end

  if CheckCollision(hero.pos_x,hero.pos_y,32,32, Shoe.pos_x,Shoe.pos_y,32,19) and Shoe.nomapa == 1 then
    Shoe.equipada = 1
    Shoe.nomapa = 0
    Shoe.pos_x = love.math.random(100,900)
    Shoe.pos_y = love.math.random(50,750)
  end

  if Shoe.equipada == 1 then
    Shoe.usagetime = Shoe.usagetime + dt
    if Shoe.usagetime > 15 then
      Shoe.equipada = 0
      Shoe.usagetime = 0
    end
  end

  if Shoe.equipada == 1 then --habilidade da arma
    hero.vel = 2.5 -- rapidez do personagem
  else
    hero.vel = hero.veloriginal
  end
  ...
  while (CheckCollision(Shoe.pos_x,Shoe.pos_y,12,12,arvore1pos_x,arvore1pos_y,arvoreverdeheight,arvoreverdewidth) or CheckCollision(Shoe.pos_x,Shoe.pos_y,12,12,arvore2pos_x,arvore2pos_y,arvoreverdeheight,arvoreverdewidth) or CheckCollision(Shoe.pos_x,Shoe.pos_y,12,12,arvore3pos_x,arvore3pos_y,arvoreverdeheight,arvoreverdewidth) or CheckCollision(Shoe.pos_x,Shoe.pos_y,12,12,arvore4pos_x,arvore4pos_y,arvoreverdeheight,arvoreverdewidth) or CheckCollision(Shoe.pos_x,Shoe.pos_y,12,12,arvorerosapos_x,arvorerosapos_y,arvorerosaheight,arvorerosawidth)) do
    Shoe.pos_x = love.math.random(100,1000)
    Shoe.pos_y = love.math.random(50,700)
  end
